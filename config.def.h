...
#else
static const char *fonts[]               = { "dina:size=8", "JoyPixels:pixelsize=10:antialias=true:autohint=true" };
...
static const Rule rules[] = {
	...
	/*RULE(.class = "Gimp", .tags = 1 << 4)
	RULE(.class = "Firefox", .tags = 1 << 7)*/
	RULE(.class = "Alacritty", .isterminal = 1, .tags = 1 << 2, .monitor = 0)
...
static const Key keys[] = {
	/* modifier                     key            function                argument */
  { ControlMask,									XK_Print,			 spawn,									 SHCMD("dunstify \"Selecting area for screenie..\" && maim -s $HOME/screenies/pic-select-$(date '+%y%m%d-%H%M-%S').png && dunstify \"Saved pic-select-$(date '+%y%m%d-%H%M-%S').png\"") },  
	{ 0,														XK_Print,			 spawn,									 SHCMD("maim -i $(xdotool getactivewindow) $HOME/screenies/pic-$(date '+%y%m%d-%H%M-%S').png && dunstify \"Saved pic-$(date '+%y%m%d-%H%M-%S').png\"") },  	
